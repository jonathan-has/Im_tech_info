...
export const Sidebar = ({close}) => {
  ...
  return (
    <div >
        <FaX size={25} className='lg:hidden block text-white m-1' onClick={close}/>
        ...
        <div className='m-5 flex flex-col gap-5'>
            ...
            <Link to='/Dashboard/Superadmin/Utilisateurs'><div className='m-1 flex gap-2 text-bold text-white duration-300 p-2 rounded-2xl hover:bg-blue-600 active:scale-95'><FaUserFriends size={25} className='text-white'/>Etudiants</div></Link>
            ...
            <div className='m-1 flex gap-2 text-bold text-white duration-300 p-2 rounded-2xl hover:bg-blue-600 active:scale-95 cursor-pointer' onClick={Deconnexion}><FaArrowRightFromBracket size={25} className='text-white'/>Déconnexion</div>
        </div>
    </div>
  )
}
export const Sidebar_ens = ({close}) => {
    const navigate = useNavigate();
    const Deconnexion = () => {
        deconnexion();
        navigate('/');
